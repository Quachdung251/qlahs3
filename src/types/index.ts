...
// Component Placeholder cho Mã QR
// Trong một ứng dụng React thực tế, bạn sẽ sử dụng thư viện như 'qrcode.react'
// Ví dụ: import QRCode from 'qrcode.react';
const QRCodeSVG = ({ value, size = 200 }) => {
  // Đây là một placeholder. Trong ứng dụng thực tế, thư viện QR code sẽ tạo ra mã QR thật.
  // Chúng ta sử dụng một URL ảnh placeholder để minh họa.
  // URL này sẽ hiển thị văn bản "QR Code for [value]" trên ảnh.
  const placeholderText = `QR Code for\\n${encodeURIComponent(value)}`;
  const placeholderImageUrl = `https://placehold.co/${size}x${size}/000000/FFFFFF?text=${placeholderText}`;

  return (
    <div className="flex flex-col items-center justify-center p-4 bg-white rounded-lg shadow-inner">
      <img src={placeholderImageUrl} alt="Mã QR Placeholder" className="rounded-md" />
      <p className="mt-2 text-sm text-gray-600 break-words text-center">
        Dữ liệu QR: <span className="font-semibold">{value}</span>
      </p>
      <p className="text-xs text-gray-500 mt-1">
        (Trong ứng dụng thực tế, thư viện QR code sẽ tạo ra mã QR thật.)
      </p>
    </div>
  );
};
...
const App: React.FC = () => {
  ...
  // States mới cho tính năng tạo QR
  const [showQrCodeModal, setShowQrCodeModal] = useState(false);
  const [qrCodeDataForDisplay, setQrCodeDataForDisplay] = useState('');
  ...
  const handleQrScanSuccess = useCallback((qrData: string) => {
    ...
    console.log('QR Data received (raw):', qrData);
    const caseId = qrData.trim(); // Loại bỏ khoảng trắng thừa
    console.log('QR Data (trimmed) to search for:', caseId);

    // Log tất cả các ID vụ án hiện có để so sánh
    console.log('Current available Case IDs in App:', cases.map(c => c.id));

    // Tìm vụ án
    const foundCase = cases.find(c => {
      console.log(`Comparing "${c.id}" with "${caseId}"`);
      return c.id === caseId;
    });
    ...
    if (foundCase) {
      ...
      setScanMessage(`Đã tìm thấy vụ án: ${foundCase.name}. Đang mở để chỉnh sửa.`);
      setTimeout(() => setScanMessage(null), 5000);
    } else {
      console.warn(`Không tìm thấy vụ án với ID: "${caseId}"`);
      setScanMessage(`Không tìm thấy vụ án với ID: "${caseId}". Vui lòng kiểm tra lại ID hoặc đồng bộ dữ liệu.`);
      setTimeout(() => setScanMessage(null), 5000);
    }
    console.log('--- End QR Scan Processing ---');
  }, [cases, handleEditCase]); // Thêm cases và handleEditCase vào dependency array

  // Hàm mới để xử lý việc tạo mã QR cho một vụ án
  const handleGenerateQrCode = useCallback((caseId: string) => {
    setQrCodeDataForDisplay(caseId);
    setShowQrCodeModal(true);
    console.log(`Generating QR code for Case ID: ${caseId}`);
  }, []);
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && user && supabase) {
      ...
      console.log(`App.tsx: Attempting to subscribe to Realtime channel: ${channelName}`);
      ...
      realtimeChannel.on(
        ...
        (payload: any) => {
          console.log('App.tsx: Received Realtime scan_event (raw payload):', payload);
          if (payload.payload && payload.payload.qrData) {
            handleQrScanSuccess(payload.payload.qrData); // Gọi hàm xử lý QR code
          } else {
            console.warn('App.tsx: Realtime payload missing qrData:', payload);
          }
        }
      ).subscribe((status: any) => {
        if (status === 'SUBSCRIBED') {
          console.log(`App.tsx: Successfully SUBSCRIBED to Realtime channel: ${channelName}`);
        } else if (status === 'CHANNEL_ERROR') {
          console.error(`App.tsx: ERROR subscribing to Realtime channel: ${channelName}`);
        } else if (status === 'TIMED_OUT') {
          console.warn(`App.tsx: Realtime channel subscription TIMED OUT: ${channelName}`);
        } else {
          console.log(`App.tsx: Realtime channel status: ${status} for channel: ${channelName}`);
        }
      });
    }
    ...
    return () => {
      if (realtimeChannel) {
        console.log(`App.tsx: Unsubscribing from Realtime channel: ${realtimeChannel.name}`);
  ...
  const getCaseTableColumns = (tabId: string, onGenerateQr: (caseId: string) => void) => {
    const baseColumns = [
          ...
          { key: 'trialTransferDate' as const, label: 'Ngày chuyển XX' },
          {
            ...
            render: (caseItem: Case) => (
                <div className="flex space-x-2">
                    ...
                    >
                        <X size={16} /> {/* Sử dụng icon X cho chỉnh sửa */}
                    </button>
                    <button
                        onClick={() => onDeleteCase(caseItem.id)} // Giả định onDeleteCase được truyền xuống
                        className="p-1 rounded-md text-red-600 hover:bg-red-100"
                        title="Xóa vụ án"
                    >
                        <X size={16} /> {/* Sử dụng icon X cho xóa */}
                    </button>
                    <button
                        onClick={() => onGenerateQr(caseItem.id)} // Nút tạo QR
                        className="p-1 rounded-md text-purple-600 hover:bg-purple-100"
                        title="Tạo mã QR cho vụ án"
                    >
                        <QrCode size={16} />
                    </button>
                </div>
  ...
  // Component Modal hiển thị Mã QR
  const QrCodeDisplayModal = ({ qrData, onClose }) => {
    if (!qrData) return null; // Không render nếu không có dữ liệu

    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white p-6 rounded-lg shadow-xl max-w-sm w-full mx-auto text-center relative">
          <button
            onClick={onClose}
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-4">Mã QR Vụ Án</h3>
          <div className="flex justify-center mb-4">
            <QRCodeSVG value={qrData} size={200} /> {/* Sử dụng component placeholder */}
          </div>
          <p className="text-sm text-gray-700 break-words mb-4">
            Vui lòng quét mã QR này bằng ứng dụng quét QR của bạn.
          </p>
          <button
            onClick={onClose}
          ...
          return (
            <>
              ...
              <CaseTable
                ...
                columns={getCaseTableColumns(activeTab, handleGenerateQrCode)} // Truyền handleGenerateQrCode
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Modal hiển thị Mã QR cho vụ án */}
      {showQrCodeModal && (
        <QrCodeDisplayModal
          qrData={qrCodeDataForDisplay}
